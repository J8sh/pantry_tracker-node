...
app.get('/', (req, res) => {
  if(req.session.user) {
    res.send( new Response(HttpStatus.OK.code, HttpStatus.OK.status, 'Ingredient API, v1.0.0 - All Systems Go - User Logged In' + JSON.stringify(req.session.user)));
  } else {
    res.send( new Response(HttpStatus.OK.code, HttpStatus.OK.status, 'Ingredient API, v1.0.0 - All Systems Go - User Not Logged In'));
  }
});
// routes not created
